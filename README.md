...
## RAG for feedback Search

The Marketing Agent uses **LlamaIndex + BAAI/bge-small-en-v1.5 embeddings** to 
semantically search user feedback rather than passing all 35 entries into every 
LLM prompt — demonstrating the RAG pattern used in production feedback analysis systems.

**How it works:**
- Each feedback entry is converted to a vector and stored in a `VectorStoreIndex`
- When the Marketing Agent queries (e.g. *"payment complaints?"*), only the top-K 
  semantically similar entries are retrieved and sent to the LLM
- This keeps prompts focused and scales to thousands of entries without code changes

**Key distinction from keyword search:** An entry mentioning *"checkout keeps timing 
out"* is retrieved for a payment query even though it contains no payment keywords — 
because the vectors capture meaning, not just words.

> To scale from 35 → 35,000 entries: swap the in-memory index for a persistent 
> vector DB (Pinecone, ChromaDB) — the agent code stays identical.
**Execution Flow:**
